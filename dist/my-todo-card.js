        ...
        for (const entity of entities) {
            try {
                ...
                const filteredItems = items.filter(item => {
                    if (!show_completed && item.status !== 'needs_action')
                        return false;
                    if (item.due) {
          ...
          <div>
            ...
            ${filteredItems.length
                    ? x `<ul>${filteredItems.map(item => x `<li>${item.summary}</li>`)}</ul>`
                    : x `<p>No items found.</p>`}
          </div>
        `);
            }
            catch (error) {
...
__decorate([
    n({ attribute: false })
], MyTodoCard.prototype, "hass", void 0);
__decorate([
    n()
], MyTodoCard.prototype, "config", void 0);
